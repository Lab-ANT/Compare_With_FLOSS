...
stepSize = 3; 
fileID = fopen(['output_FLOSS\UCR-SEG_segpos.txt'],'w');
path = 'data\UCR-SEG\UCR_datasets_seg\*.txt';
%fprintf(fileID,'file name , predictedSegments , score\n');
[vals, fnames, numfids] = readFiles(path);
mkdir('output_FLOSS\crosscount\UCR-SEG\');
...
for filesInd = 1:numfids
    ...
    for colInd = 1:ncols
        ...
        dlmwrite(['output_FLOSS\crosscount\UCR-SEG\',fnames(filesInd,:)], crosscount');
%         writematrix(crosscount', ['output_FLOSS\crosscount\UCR-SEG\',fnames(filesInd,:)]);
    end
end
fclose(fileID);
